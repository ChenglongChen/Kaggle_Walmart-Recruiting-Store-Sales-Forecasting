...
cat('Totally 5 set of features ...\n')


#### Indicate which week of the date is in the corresponding month
# verbose
cat('Generate feature set 1 ...\n')
dfTrain$Week_in_Month <- as.factor(floor(dfTrain$Day/7))
dfTest$Week_in_Month <- as.factor(floor(dfTest$Day/7))

# verbose
cat('Done\n')


#### Indicate which season of the year it is
cat('Generate feature set 2 ...\n')
dfTrain$Season <- as.factor(floor(as.numeric(dfTrain$Month)/3))
dfTest$Season <- as.factor(floor(as.numeric(dfTest$Month)/3))
...
dfTrain$Days_From_Last_Holiday_laplace1 <- exp(-dfTrain$Days_From_Last_Holiday/diversity1)
dfTrain$Days_From_Last_Holiday_laplace2 <- exp(-dfTrain$Days_From_Last_Holiday/diversity2)
dfTrain$Days_From_Last_Holiday_laplace4 <- exp(-dfTrain$Days_From_Last_Holiday/diversity4)
dfTrain$Days_From_Last_Holiday_laplace7 <- exp(-dfTrain$Days_From_Last_Holiday/diversity7)
#dfTrain$Days_From_Last_Holiday_laplace14 <- exp(-dfTrain$Days_From_Last_Holiday/diversity14)
#dfTrain$Days_From_Last_Holiday_laplace21 <- exp(-dfTrain$Days_From_Last_Holiday/diversity21)
...
dfTrain$Days_To_Next_Holiday_laplace1 <- exp(-dfTrain$Days_To_Next_Holiday/diversity1)
dfTrain$Days_To_Next_Holiday_laplace2 <- exp(-dfTrain$Days_To_Next_Holiday/diversity2)
dfTrain$Days_To_Next_Holiday_laplace4 <- exp(-dfTrain$Days_To_Next_Holiday/diversity4)
dfTrain$Days_To_Next_Holiday_laplace7 <- exp(-dfTrain$Days_To_Next_Holiday/diversity7)
#dfTrain$Days_To_Next_Holiday_laplace14 <- exp(-dfTrain$Days_To_Next_Holiday/diversity14)
#dfTrain$Days_To_Next_Holiday_laplace21 <- exp(-dfTrain$Days_To_Next_Holiday/diversity21)
...
dfTest$Days_From_Last_Holiday_laplace1 <- exp(-dfTest$Days_From_Last_Holiday/diversity1)
dfTest$Days_From_Last_Holiday_laplace2 <- exp(-dfTest$Days_From_Last_Holiday/diversity2)
dfTest$Days_From_Last_Holiday_laplace4 <- exp(-dfTest$Days_From_Last_Holiday/diversity4)
dfTest$Days_From_Last_Holiday_laplace7 <- exp(-dfTest$Days_From_Last_Holiday/diversity7)
#dfTest$Days_From_Last_Holiday_laplace14 <- exp(-dfTest$Days_From_Last_Holiday/diversity14)
#dfTest$Days_From_Last_Holiday_laplace21 <- exp(-dfTest$Days_From_Last_Holiday/diversity21)
...
dfTest$Days_To_Next_Holiday_laplace1 <- exp(-dfTest$Days_To_Next_Holiday/diversity1)
dfTest$Days_To_Next_Holiday_laplace2 <- exp(-dfTest$Days_To_Next_Holiday/diversity2)
dfTest$Days_To_Next_Holiday_laplace4 <- exp(-dfTest$Days_To_Next_Holiday/diversity4)
dfTest$Days_To_Next_Holiday_laplace7 <- exp(-dfTest$Days_To_Next_Holiday/diversity7)
#dfTest$Days_To_Next_Holiday_laplace14 <- exp(-dfTest$Days_To_Next_Holiday/diversity14)
#dfTest$Days_To_Next_Holiday_laplace21 <- exp(-dfTest$Days_To_Next_Holiday/diversity21)
...
#### What is the weekly sales of last year?
# ------------------------------------------------------------------------------
# These features are motivated as the times series of weekly sales appear highly
# periodic. For some depts, e.g., dept=1, you can simply predict the next year
# weekly slaes using the corresponding weekly sales last year. (If you look at
# the ts plot of weekly sales, it sounds quite convincing to do so).
# ------------------------------------------------------------------------------
# verbose
cat('Generate feature set 5 ...\n')
...
## temp variable
dfTest2 <- dfTest
dfTest2$Weekly_Sales <- rep(NA, dim(dfTest2)[1])
dfAll <- rbind(dfTrain, dfTest2)
...
dfTrain$Last_Year_Sales51 <- results
# offset = 52
results <- with(dfAll,
                by(data=dfAll,
                   INDICES=list(Store, Dept),
...
results <- unlist(results)
dfTrain$Last_Year_Sales52 <- results
# offset = 53
results <- with(dfAll,
                by(data=dfAll,
                   INDICES=list(Store, Dept),
...
results <- unlist(results)
dfTrain$Last_Year_Sales53 <- results
...
results <- with(dfAll,
                by(data=dfAll,
                   ...
                   FUN=function(df)sapply(df$Day_Index[df$Day_Index>143],
                                          function(d)func(d, df, offset=51))))
results <- unlist(results)
dfTest$Last_Year_Sales51 <- results
# offset = 52
results <- with(dfAll,
                by(data=dfAll,
                   INDICES=list(Store, Dept),
                   FUN=function(df)sapply(df$Day_Index[df$Day_Index>143],
                                          function(d)func(d, df, offset=52))))
results <- unlist(results)
dfTest$Last_Year_Sales52 <- results
# offset = 53
results <- with(dfAll,
                by(data=dfAll,
                   INDICES=list(Store, Dept),
                   FUN=function(df)sapply(df$Day_Index[df$Day_Index>143],
                                          function(d)func(d, df, offset=53))))
results <- unlist(results)
dfTest$Last_Year_Sales53 <- results
...
save(list=c('dfTrain', 'trainWeights', 'dfTest', 'testID', 'dfSampleSubmission'),
     file='./data/training_testing_data_v2.RData')
